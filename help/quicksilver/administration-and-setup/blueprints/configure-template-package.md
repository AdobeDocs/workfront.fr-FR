...
1. Passez aux sections suivantes :
   ...
   * [[!UICONTROL Mappage des rôles]](#role-mapping)
   * [[!UICONTROL Mappage des équipes]](#team-mapping)
   * [[!UICONTROL Mappage d’entreprise]g](#company-mapping)
...
1. Dans le [!UICONTROL Préférences de modèle] , indiquez un nouveau nom de modèle.
...
1. Indiquez si le modèle est installé comme principal ou inactif.
1. Choisissez si vous souhaitez utiliser les nouvelles préférences de problème définies, si des préférences sont disponibles.
   ...
   Cliquez sur **[!UICONTROL Voir Préférences de problème]** pour consulter les préférences spécifiques qui s’installeront avec le plan directeur. Les projets créés à partir du modèle importé utilisent ces préférences pour les nouveaux problèmes ajoutés dans le [!UICONTROL Problèmes] .
   ...
   <table style="table-layout:auto"> 
    ...
    <tbody> 
     <tr> 
      ...
      <td> <p>Les groupes de rubriques de file d’attente définissent le niveau de catégories le plus élevé pour les problèmes ou les requêtes. Les utilisateurs voient les groupes de rubriques comme des options de menu lors de la sélection de l’emplacement d’envoi des requêtes. Un groupe de rubriques peut contenir plusieurs rubriques de file d’attente. Pour plus d’informations, voir <a href="../../manage-work/requests/create-and-manage-request-queues/create-topic-groups.md" class="MCXref xref">Création de groupes de rubriques</a>. </p> </td> 
     </tr> 
     <tr> 
      <td role="rowheader"><strong>Rubriques en file d’attente</strong></td> 
      <td> <p>Les rubriques de file d’attente fonctionnent conjointement avec les règles de routage pour affecter des problèmes ou des requêtes. Il s’agit des options de menu que les utilisateurs sélectionnent lorsqu’ils saisissent un problème ou une requête, après avoir sélectionné un groupe de rubriques. Pour plus d’informations, voir <a href="../../manage-work/requests/create-and-manage-request-queues/create-queue-topics.md" class="MCXref xref">Création de rubriques de file d’attente</a>. </p> </td> 
     </tr> 
     <tr> 
   ...
   >![Exemples de préférences de problème](assets/Blueprints_IssuePrefsDetails.png)
   ...
   >* La définition de ces préférences ne place pas automatiquement les projets créés à partir du modèle dans des files d’attente de requêtes. Pour plus d’informations sur la configuration d’une file d’attente de requêtes, voir [Création d’une file d’attente de requête](../../manage-work/requests/create-and-manage-request-queues/create-request-queue.md).
...
![[!UICONTROL Mappage des rôles] section](assets/Blueprints_RoleMapping.png)
...
![[!UICONTROL Mappage d’équipe] section](assets/Blueprints_TeamMapping.png)
...
1. Si une équipe existe, vous pouvez choisir l’une des options suivantes :
   ...
   1. Utilisation [!UICONTROL équipe existante], puis sélectionnez une équipe dans la zone de sélection.
...
Pour plus d’informations sur le fonctionnement des équipes dans [!DNL Workfront], voir [Création et gestion des équipes](../../people-teams-and-groups/create-and-manage-teams/create-and-mange-teams.md).
...
![[!UICONTROL Mappage d’entreprise] section](assets/Blueprints_CompanyMapping.png)
...
Pour plus d’informations sur l’association d’un modèle à une société, voir [Modifier des modèles de projet](../../manage-work/projects/create-and-manage-templates/edit-templates.md).

Pour plus d’informations sur le fonctionnement des entreprises dans [!DNL Workfront], voir [Création et modification d’entreprises](../../administration-and-setup/set-up-workfront/organizational-setup/create-and-edit-companies.md).
...
![[!UICONTROL Mappage de groupe] section](assets/Blueprints_GroupMapping.png)

1. Si un groupe existe, vous pouvez sélectionner **[!UICONTROL Remap Group]** et sélectionnez l’une des options suivantes :

   1. **[!UICONTROL Création d’un groupe portant un nom différent]**, puis saisissez le nom à affecter à ce groupe. Les références au groupe dans la définition de plan directeur seront associées à ce nouveau groupe à la place.
   1. **[!UICONTROL Remplacer par un groupe existant]**, puis recherchez et sélectionnez un groupe dans la zone de sélection.
...
1. Si un groupe n’existe pas, vous pouvez :
   ...
   1. Sélectionner **[!UICONTROL Remap Group]** et choisissez [!UICONTROL Remplacer par un groupe existant], puis recherchez et sélectionnez un groupe dans la zone de sélection.
   1. Sélectionner **[!UICONTROL Remap Group]** et choisissez **[!UICONTROL Insérer sous un groupe existant]**, puis recherchez et sélectionnez un groupe dans la zone de sélection. Cette option crée un sous-groupe sous le groupe existant.
