...
>
>* Si vous déplacez une tâche ou un problème et que vous choisissez de ne pas déplacer également les documents joints à la tâche ou au problème, les documents sont supprimés et placés dans la Corbeille pendant 30 jours. Un administrateur peut les restaurer et il sera rattaché à la tâche ou au problème déplacé. Si la tâche ou le problème a été supprimé depuis qu’ils ont été déplacés, les documents seront restaurés dans la zone Documents de la page utilisateur de l’administrateur qui les restaure.
...
## Informations récupérées lors de la restauration d’un projet, d’une tâche ou d’un problème
...
* URL de l’élément
   ...
   Une fois restauré, l’URL de l’élément reste identique. Si des utilisateurs ont créé des signets de navigateur pour l’élément, ils restent valides.
...
## Informations non récupérées lors de la restauration d’un projet, d’une tâche ou d’un problème
...
1. Cliquez sur le bouton **Projets**, **Tâches**, **Problèmes** ou **Documents** selon le type d’élément à restaurer.
...
1. (Facultatif) Pour afficher rapidement l’élément restauré, suivez les étapes de la section [Afficher l’élément restauré](../../../administration-and-setup/manage-workfront/manage-deleted-items/view-restored-items.md).
   ...
   Pour plus d’informations sur ce qui se passe après la restauration d’un élément, voir [Que se passe-t-il après la restauration des éléments ?](#what-happens-after-you-restore-items) dans cet article.

## Que se passe-t-il après la restauration des éléments ? {#what-happens-after-you-restore-items}

* Lorsque vous restaurez des tâches et des sous-tâches, elles apparaissent dans l’ordre dans lequel elles avaient été supprimées avant d’être supprimées.

   Cependant, si l’ordre des autres tâches change lorsque la tâche est supprimée, la tâche peut être restaurée au bas de la liste des tâches ou sous-tâches.

* Après avoir restauré un élément :

   * Un message s’affiche pour vous indiquer si vous avez réussi.

      Vous recevez également une notification électronique. Si vous avez restauré plusieurs éléments, le courrier électronique les répertorie.
   ...
   * Un commentaire s’affiche dans la zone Mises à jour du projet, de la tâche ou du problème, ainsi que dans celle de l’objet parent.
      ...
      Cela ne se produit pas lorsque vous restaurez un document.

## BAT restauré

Actuellement, lorsqu’une personne restaure un document comportant un BAT, la page Activités de contrôle du BAT peut afficher le nom du premier administrateur Workfront principal répertorié pour l’instance de votre organisation (par ordre d’identifiant de profil) au lieu de la personne qui a restauré le BAT.
