...
description: Les Kickstarts sont des classeurs Excel spécialement formatés que vous pouvez renseigner avec les données à importer dans Workfront. Adobe Workfront fournit un modèle Kickstart que vous pouvez utiliser pour ce faire, comme expliqué dans la section Importateur de données Kickstart.
author: Lisa
...
Lorsque vous exportez un modèle Kickstart, vous recevez un classeur de feuille de calcul Excel vierge. Une fois la feuille de calcul téléchargée sur votre ordinateur, vous pouvez l’utiliser pour remplir vos informations, puis la réimporter dans Workfront.
...
1. Continuez vers [Remplir le modèle de feuille de calcul avec vos données](#populate-the-spreadsheet-template-with-your-data) pour remplir le modèle de feuille de calcul vierge avec vos informations.
...
* [Substitution des identifiants par des noms d’attributs](#attribute-name-substitution-for-ids)

### Vue d’ensemble des onglets (feuilles de données) inclus dans la feuille de calcul

>[!TIP]
>
>Pour mieux comprendre comment vous devez mettre en forme les informations dans chaque colonne lorsque vous remplissez le modèle Kick-Start, envisagez d’effectuer une exécution à des fins d’entraînement en exportant un Kick-Start avec des données Workfront existantes dans les objets que vous essayez d’importer. Pour obtenir des instructions, consultez [Exporter des données à partir d’Adobe Workfront via Kick-Starts](../../../administration-and-setup/manage-workfront/using-kick-starts/export-data-from-wf-via-kick-starts.md).

Lorsque vous ouvrez un modèle Kick-Starts vierge, plusieurs onglets (feuilles de données) sont disponibles. Ils dépendent des objets que vous avez sélectionnés pour être téléchargés. Chacun d’entre eux représente un objet dans l’application, tel que le projet, les tâches, les heures, le tableau de bord et les utilisateurs et les utilisatrices :

Lorsque vous ouvrez l’un de ces onglets, la deuxième ligne affiche les champs de chaque objet qui peuvent être paramétrés au cours d’un import. Dans un en-tête de colonne, après le mot « set », le nom du champ s’affiche tel qu’il apparaît dans la base de données. Ces champs se comportent comme des en-têtes de colonne.

>[!IMPORTANT]
>
>Pour éviter des erreurs, respectez les points suivants :
>
>* Ne supprimez pas la première ligne vide d’une feuille de calcul Kick-Start.
>* Ne supprimez pas, ne modifiez pas et ne réorganisez pas ces champs (en-têtes de colonne) de quelque manière que ce soit. Par exemple, ne modifiez pas leur ordre ou leur nom.
>* Ajoutez des valeurs à chaque champ qui s’affiche en gras dans l’en-tête de colonne. Il s’agit de champs obligatoires.
>
...
>
>     Par exemple, dans l’onglet de **projet PROJ**, les champs **setCondition** et **setConditionType** peuvent être laissés vides, mais pas les colonnes **setGroupID** et **setName**.
>
>* Certains champs, dont **setResourceRevenue** et **setEnteredByID**, sont générés automatiquement par le système. Si vous saisissez des données pour ces champs dans la feuille de calcul, le processus Kick-Start les remplacera lorsque vous chargerez la feuille de calcul.
...
1. Ajoutez des informations dans la colonne **ID** de l’une des manières suivantes :
   ...
   * Si l’objet que vous importez est nouveau (et que vous avez saisi **TRUE** dans la colonne **isNew**), saisissez n’importe quel numéro pour l’ID. Ce nombre doit être unique dans la feuille de calcul. Par exemple, si vous importez trois objets, vous pouvez leur attribuer respectivement les ID 1, 2 et 3.
   ...
   * Vous pouvez importer un nouvel objet basé sur un objet existant en remplaçant **FALSE** par **TRUE** dans la colonne **isNew**, en modifiant l’ID et en ajustant les données avant l’import.
   ...
   * Lorsque vous importez un projet, vous devez indiquer un ID de groupe.
      ...
      * Si le groupe n’existe pas dans Workfront, vous pouvez ajouter la feuille de groupe GROUP dans votre fichier d’import, définir le champ isNew sur TRUE dans la feuille de groupe, puis indiquer un ID numérique pour le nouveau groupe dans la colonne ID. **************** Le champ **setGroupID** du nouveau projet doit correspondre à l’**ID** numérique du nouveau groupe.

     **Exemple :** pour un projet, la valeur affichée dans la colonne **setGroupID** doit correspondre à l’un des éléments suivants :

      * Le GUID d’un groupe existant dans votre instance Workfront.
      * La valeur (nombre) dans la colonne ID de la feuille de **groupe GROUP** si vous créez un nouveau groupe au cours de l’import.

1. Saisissez des valeurs pour les champs obligatoires et pour tout autre champ que vous souhaitez renseigner lors de l’import.
1. (Facultatif) Pour ajouter des données personnalisées, procédez comme suit :
   ...
   * Nommez chaque nouvelle colonne en fonction de son champ personnalisé correspondant comme suit : DE:Nom du champ personnalisé tel qu’il apparaît dans Workfront. **[]** Par exemple, vous pouvez créer le champ personnalisé suivant : « DE:Services ».
   * Dans la colonne setCategoryID, saisissez le GUID du formulaire personnalisé existant dans lequel se trouve ce champ personnalisé. **** Ce champ est obligatoire lors de l’import de données personnalisées.
   * Si vous devez ajouter plusieurs valeurs de données dans un champ personnalisé (comme des cases d’option, des cases à cocher ou des listes), utilisez le délimiteur de données personnalisées en forme de barre verticale « | » répertorié dans l’onglet Préférences pour séparer les valeurs.

     **Exemple :** saisissez A|D dans la colonne DE:Services pour renseigner les services A et D dans votre formulaire personnalisé.
...
Par exemple : 07/10/2022 01:30 ou 07/10/2022 1:00 PM.

Si vous n’incluez pas d’heure avec la date, Workfront effectue l’une des opérations suivantes :

* Workfront suppose que l’heure est 12:00 AM (minuit). Pour que vous obteniez la date correcte, le fuseau horaire du système doit correspondre à votre fuseau horaire.
* Si la date est sur un objet associé à un planning, l’heure est décalée au moment le plus tôt autorisé par le planning.
...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  ...
  <tr> 
   <td> <p>$$CUSTOMER</p> </td> 
   <td> <p>Ce caractère générique a été ajouté spécifiquement pour les imports des utilisateurs et utilisatrices Kickstart. La création d’un compte Workfront génère la création d’un utilisateur ou d’une utilisatrice disposant d’un niveau d’accès Administration système. Le nom d’utilisateur ou d’utilisatrice attribué à l’administrateur ou à l’administratrice par défaut peut être utilisé comme préfixe lors de la création d’autres utilisateurs et d’autres utilisatrices dans le compte.</p> <p>Les noms d’utilisateur ou d’utilisatrice devant être uniques pour toute la clientèle, cette option est très utile lorsque vous avez plusieurs personnes avec des noms d’utilisateurs et d’utilisatrices très courants, tels que John Smith, dont le nom d’utilisateur peut être « jsmith ». En ajoutant l’affectation du nom d’utilisateur ou d’utilisatrice en préfixe au nom par défaut affecté à l’administration, vous garantissez que chaque nom d’utilisateur ou d’utilisatrice est unique (par exemple : <strong>$$CUSTOMER.jsmith</strong>).</p> <p>Conseil : pour vous assurer que les noms des utilisateurs et des utilisatrices sont uniques à l’échelle du système, saisissez l’adresse e-mail de la personne dans le champ <strong>setUsername</strong>.</p> </td> 
  </tr> 
 </tbody> 
</table>
...
Même si la bonne pratique est d’utiliser des ID lorsque cela est possible, il n’est pas toujours pratique de les transférer d’une feuille à l’autre lors de la configuration de la valeur d’un setAttributeID. **** Vous pouvez référencer des valeurs par nom en modifiant simplement l’en-tête de colonne.
...
* **Import de projets**
  ...
  Cette méthode fonctionne bien avec un faible nombre de groupes et de projets. Elle est cependant peu pratique lorsque le nombre de projets et de groupes est plus élevé.

  Pour substituer le nom de l’attribut dans l’exemple décrit ci-dessus, vous devez remplacer l’en-tête de colonne **setGroupID** par le **nom #setGroupID GROUP**. Vous pouvez ensuite référencer chaque groupe de chaque projet par son nom.
...
* **Import d’utilisateurs ou d’utilisatrices**
  ...
  Pour les nouveaux enregistrements d’utilisateurs et d’utilisatrices affectés à des rôles existants, vous pouvez utiliser la substitution de noms. Pour les nouveaux enregistrements d’utilisateurs et d’utilisatrices affectés à des rôles nouvellement importés, il est impossible d’utiliser la substitution de noms.
   ...
   * Pour des affectations de rôle à des enregistrements existants, saisissez les noms de rôle dans la colonne avec le **nom #setRoleID ROLE**.

     Pour des affectations de rôles à des nouveaux enregistrements de rôle, saisissez l’ID que vous avez affecté dans la feuille de rôle « ROLE » dans « setRoleID ».
...
Une fois que vous avez renseigné le modèle Excel avec vos données, vous pouvez charger les données de ce dernier dans Workfront.

L’import Kickstart prend en charge les types de fichiers suivants :
