...
* Rapport
* Plan des effectifs
* Valeur du paramètre du plan des effectifs
* Ressource du plan des effectifs
* Valeur de l’attribut de ressource du plan des effectifs
...
## Authentification des abonnements aux événements
...
## Création de la ressource d’abonnement
   ...
   * **Chaîne** : code d’objet de l’objet concerné par l’abonnement aux modifications. Les valeurs possibles pour objCode sont répertoriées dans le tableau ci-dessous.
     ...
     <table style="table-layout:auto"> 
      ...
      <tbody> 
       ...
       <tr> 
        <td scope="col">Participant ou participante à l’étape d’approbation</td> 
      ...
      <tr> 
       ...
       <tr> 
        <td scope="col"><p>Plan des effectifs</p></td> 
        <td scope="col"><p>STAFFP</p></td> 
       </tr> 
       <tr> 
        <td scope="col"><p>Valeur du paramètre du plan des effectifs</p></td> 
        <td scope="col"><p>SPVAL</p></td> 
       </tr> 
       <tr> 
        <td scope="col"><p>Ressource du plan des effectifs</p></td> 
        <td scope="col"><p>STAFFR</p></td> 
       </tr> 
       <tr> 
        <td scope="col"><p>Valeur de l’attribut de ressource du plan des effectifs</p></td> 
        <td scope="col"><p>SPAVAL</p></td> 
       </tr> 
       <tr> 
        <td scope="col"><p>Valeur définie de l’attribut de ressource du plan des effectifs</p></td> 
        <td scope="col"><p>SAVSET</p></td> 
       </tr> 
       <tr> 
        <td scope="col"><p>Valeur du paramètre de ressource du plan des effectifs</p></td> 
...
Workfront propose deux versions des abonnements aux événements.
...
### Changement de version d’un seul abonnement

La syntaxe de requête pour modifier la version d’un seul abonnement est la suivante :
...
### Changement de version de plusieurs abonnements

Ce point d’entrée change la version de plusieurs abonnements, en utilisant la liste des abonnements ou l’indicateur de tous les abonnements du client ou de la cliente.
...
* Corps de requête pour la liste des abonnements
...
* Corps de requête pour tous les abonnements du client ou de la cliente
...
* Si la syntaxe de votre filtre est incorrecte ou ne correspond à aucune donnée contenue dans le **newState** de la payload, aucun message de validation ne sera renvoyé pour indiquer qu’une erreur s’est produite
...
Ce filtre permet aux messages d’être transmis uniquement si la totalité des valeurs sélectionnées correspond exactement à la valeur fieldValue du filtre, quel que soit l’ordre. Il ne doit pas y avoir de valeurs supplémentaires ou manquantes.

>[!NOTE]
>
>Il est utilisé pour les champs de type tableau (à sélection multiple). L’exemple d’abonnement ci-dessous autorise les messages uniquement si le champ `groups` contient exactement « Choix 3 » et « Choix 4 », sans valeurs supplémentaires ou manquantes, et quel que soit l’ordre. Si une chaîne ou un entier est spécifié dans `fieldValue` plutôt qu’un tableau, l’abonnement autorise les messages uniquement si le champ `groups` contient exactement une option et que cette option correspond exactement à la chaîne ou à l’entier spécifié dans `fieldValue`.
...
Les abonnements aux événements prennent en charge le filtrage sur des champs d’événements imbriqués à l’aide des noms de champs imbriqués. Par exemple, pour filtrer un message dans lequel `newState.data.customField1 = 'myCustomFieldValue'`, vous pouvez créer l’abonnement suivant avec un filtre :
...
Vous pouvez également utiliser des filtres doublement imbriqués.
...
Pour garantir des performances et une maintenabilité cohérentes :

* Chaque abonnement prend en charge jusqu’à 10 groupes de filtres (chaque groupe contenant plusieurs filtres).
...
* Bien que la prise en charge d’un maximum de 10 groupes de filtres (chacun avec 5 filtres) soit activée, un grand nombre d’abonnements actifs avec une logique de filtre complexe peut entraîner un retard lors de l’évaluation des événements.

Si vous vous trouvez au-delà de ces limites, pensez à simplifier votre logique ou à diviser l’abonnement en plusieurs abonnements plus petits.
...
Les groupes de filtres permettent de créer des conditions logiques (AND/OR) imbriquées dans les filtres des abonnements aux événements.
...
1. Groupe de filtres (filtres imbriqués avec OR) :
...
1. Connecteur de niveau supérieur (filterConnector : AND) :
   * Le connecteur situé le plus à l’extérieur entre les filtres de niveau supérieur est « AND ». Cela signifie que le filtre de niveau supérieur et le groupe doivent être validés pour que l’événement corresponde.

1. L’abonnement se déclenche si les conditions suivantes sont remplies :
...
>* Chaque abonnement prend en charge jusqu’à 10 groupes de filtres (chaque groupe contenant plusieurs filtres).
>* Chaque groupe de filtres peut inclure jusqu’à 5 filtres pour éviter une dégradation potentielle des performances lors du traitement des événements.
>* Bien qu’il soit possible d’avoir jusqu’à 10 groupes de filtres (chacun avec 5 filtres), un grand nombre d’abonnements actifs avec une logique de filtre complexe peut entraîner un retard lors de l’évaluation des événements.
