...
1. Modifiez l’URL en /attask/api/&lt;supported-version>/project/search.\
...
1. Remplacez l’URL par **/attask/api/&lt;supported-version>/project/search**.
...
Notez que la requête /attask/api/&lt;supported-version>/project?id=... est identique à la requête `/attask/api/&lt;supported-version&gt;/project/...`.
...
<pre>GET /attask/api/&lt;supported-version&gt;/task/search?name=Planning<br>&amp;name_Mod=contains<br>&amp;OR:1:portfolio:name=FixedAssets<br>&amp;OR:1:portfolio:name_Mod=eq<br>&amp;OR:1:assignedTo:name=Steve<br>&amp;OR:1:assignedTo:name_Mod=cicontains<br>&amp;OR:2:<br> Parent:name=Final Task:2:&amp;OR&amp;&amp;&amp;&amp;Affectationparent:name_Mod=eq
</pre>
...
<pre>/attask/api/&lt;supported-version&gt;/task/search?map=true</pre>renvoie une réponse indexée par ID similaire à ce qui suit :
...
<pre>/attask/api/&lt;supported-version&gt;/task/search?fields=scheduledStartDate,priority</pre>renvoie une réponse similaire à la suivante :
...
<pre>/attask/api/&lt;supported-version&gt;/issue/search?fields=owner</pre>Si vous avez besoin de plus d’informations, vous pouvez demander un champ imbriqué en utilisant la syntaxe des deux points. Par exemple, la requête suivante permet de rechercher tous les problèmes ainsi que le nom, l’ID, le titre et le numéro de téléphone de la personne propriétaire.
<pre>/attask/api/&lt;supported-version&gt;/issue/search?fields=owner:title,owner:phoneNumber</pre>et de renvoyer ce qui suit : 
...
<pre>/attask/api/&lt;supported-version&gt;/project/search?fields=tasks</pre>La requête suivante récupère des affectations de tâches :
...
<pre>/attask/api/&lt;supported-version&gt;/project/search?fields=DE:CustomText</pre>qui renvoie
...
<pre>/attask/api/&lt;supported-version&gt;/project/search?fields=parameterValues</pre>renvoie des données similaires à celles qui suivent :
...
<pre>GET /attask/api/&lt;supported-version&gt;/project/count?status=CUR</pre>renvoie le nombre de résultats dans le format suivant :
...
<pre>GET /attask/api/&lt;supported-version&gt;/hour/report?project:name_1_GroupBy=true&amp;hours_AggFunc=sum</pre>qui renvoie le résultat suivant
...
<pre>GET /attask/api/&lt;supported-version&gt;/project/123abcxxxxxxxxxxxxxxxxxxxxxx?method=put &amp;updates={ accessRules: [ {accessorID: 'abc123', accessorObjCode: 'USER', coreAction: 'VIEW'} ] }</pre>Il est également possible de ne partager qu’avec une nouvelle personne et de conserver intactes les autorisations existantes :
<pre>GET /attask/api/&lt;supported-version&gt;/project/123abcxxxxxxxxxxxxxxxxxxxxxx/share?method=put&amp;accessorID=abc123&amp;accessorObjCode=USER&amp;coreAction=VIEW</pre>Pour récupérer les règles d’accès existantes :
<pre>GET /attask/api/&lt;supported-version&gt;/project/123abcxxxxxxxxxxxxxxxxxxxxxx?fields=accessRules:*</pre>
...
<pre>POST /attask/api/&lt;supported-version&gt;/project?name=New Project</pre>La réponse comprend le projet qui vient d’être créé avec son nouvel ID et tous les autres champs spécifiés.
...
<pre>POST /attask/api/&lt;supported-version&gt;/upload</pre>L’API s’attend à ce que le type de contenu soit multipart/form-data. Le nom du paramètre pour le fichier doit être uploadedFile. Le serveur renvoie les données JSON suivantes :
...
<pre>POST /attask/api/&lt;supported-version&gt;/document?updates={<br> name : aFileName,<br> handle : abc...123, (handle à partir du chargement de fichier)<br> docObjCode : PROJ, (ou TASK, OPTASK, etc.)<br> objID : abc...123,<br> currentVersion :{version:v1.0,fileName:aFileName}<br>}</pre>
...
<pre>PUT /attask/api/&lt;supported-version&gt;/project/4c7...?name=New Project Name <br>PUT /attask/api/&lt;supported-version&gt;/project?id=4c7...&amp;name=New Project Name</pre>La mise à jour nécessitant un ID, cette opération échouera (sans insertion) si l’objet n’existe pas sur le serveur.
...
<pre>PUT /attask/api/&lt;supported-version&gt;/project/4c7...?updates= <br>{<br> name : « Nouveau nom du projet », <br> status : « CUR », <br> ... <br>}</pre>
...
<pre>PUT /attask/api/&lt;supported-version&gt;/task/4c7...?updated= <br>{<br> affectations : [ <br> { <br> assignedToID : « 2222...54d0, <br> assignmentPercent : 50.0 <br> },{ <br> roleID : « 111...54d0 »<br> } <br> ] <br>}</pre>
...
<pre>PUT /attask/api/&lt;supported-version&gt;/project/4c7...?action=calculateTimeline<br><br>or<br><br>PUT /attask/api/&lt;supported-version&gt;/project/4c7.../calculateTimeline </pre>

### Déplacer des objets

La syntaxe suivante permet de déplacer une tâche d’un projet à un autre :
<pre>PUT /attask/api/&lt;supported-version&gt;/task/4c7.../move?projectID=5d8...</pre>Un exemple pour chaque type d’action est fourni ici : (??)
<pre>PUT /attask/api/&lt;supported-version&gt;/project/1234/approveApproval<br><br>PUT /attask/api/&lt;supported-version&gt;/project/1234/calculateFinance<br><br>PUT /attask/api/&lt;supported-version&gt;/project/1234/calculateTimeline<br><br>PUT /attask/api/&lt;supported-version&gt;/project/1234/calculateDataExtension<br><br>PUT /attask/api/&lt;supported-version&gt;/project/1234/reminderApproval<br><br>PUT /attask/api/&lt;supported-version&gt;/project/1234/sendApproval<br><br>PUT /attask/api/&lt;supported-version&gt;/task/1234/move<br><br>PUT /attask/api/&lt;supported-version&gt;/workitem/1234/markViewed</pre>L’identification d’attributs supplémentaires est nécessaire uniquement pour l’action de déplacement afin de spécifier le projet dans lequel l’élément de travail doit être déplacé.
...
<pre>PUT /attask/api/&lt;supported-version&gt;/project/123abcxxxxxxxxxxxxxxxxxxxxxxxxxx/share?accessorID=123abcxxxxxxxxxxxxxxxxxxxxxxxxxx&amp;accessorObjCode=TEAMOB</pre>Lors de la modification d’un objet, vous pouvez remplacer toutes les règles d’accès sur un objet en effectuant une requête PUT et en envoyant des mises à jour comme dans l’exemple suivant :
<pre>PUT /attask/api/&lt;supported-version&gt;/project/123abcxxxxxxxxxxxxxxxxxxxxxx?method=PUT&amp;updates={accessRules:[{accessorID:'123abcxxxxxxxxxxxxxxxxxxxxxxxxxx',accessorObjCode:'TEAMOB',coreAction:'VIEW'}]}</pre>La syntaxe suivante permet de déplacer une tâche d’un projet à un autre :
...
<pre>DELETE /attask/api/&lt;supported-version&gt;/task/4c78821c0000d6fa8d5e52f07a1d54d0 <br>DELETE /attask/api/&lt;supported-version&gt;/task?id=4c78821c0000d6fa8d5e52f07a1d54d0 <br>DELETE /attask/api/&lt;supported-version&gt;/task/4c78821c0000d6fa8d5e52a1d54d0?force=true <br>DELETE /attask/api/&lt;supported-version&gt;/task?id=4c78821c0000d6fa8d5e52f07a1d54d0?force=true</pre>
...
<pre>PUT /attask/api/&lt;supported-version&gt;/proj?Umethod=PUT&amp;updates=[{« ID »:« 123abcxxxxxxxxxxxxxxxxxxxxxx »,« name »:« Test_Project_1_ Edit »},{« ID »:« 123abcxxxxxxxxxxxxxxxxxxxxxxxxxxxx »,« name »:« Test_Project_2_Edit »}]&amp;apiKey=123abcxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx</pre>ce qui se traduit par un retour similaire à ce qui suit :
