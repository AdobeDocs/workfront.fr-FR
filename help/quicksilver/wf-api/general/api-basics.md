...
La requête suivante recherche toutes les tâches qui ne sont pas encore terminées et qui sont affectées à un utilisateur nommé Jean.
...
L’utilisation de paramètres d’URL pour les filtres de recherche peut présenter l’inconvénient suivant : Workfront analyse certains paramètres avant de rechercher différentes méthodes d’authentification (par exemple, nom d’utilisateur, mot de passe, apiKey, cookie). Dans ce cas, les paramètres ne sont pas utilisés comme filtres dans l’appel à . 

Pour éviter ce problème, vous pouvez placer ces valeurs dans des paramètres de filtre au format JSON. Par exemple, si vous souhaitez filtrer l’utilisateur ou l’utilisatrice qui teste le nom d’utilisateur, au lieu d’utiliser 
...
Par défaut, les données renvoyées par une recherche sont un tableau JSON. Selon votre cas d’utilisation, il peut être plus efficace d’obtenir le résultat sous la forme d’un objet JSON indexé par ID. Pour ce faire, utilisez le paramètre de requête de mappage . Par exemple, la requête . 
<pre>/attask/api/v15.0/task/search?map=true</pre>renvoie une réponse indexée par un identifiant semblable à ce qui suit :
<pre>{<br>    « data » : {<br>        « 4c9a97db0000000f13ee4446b9aead9b » : {<br>            « percentComplete » : 0,<br>            « status »: « NEW »,<br>            « name » : « première tâche »,<br>            « ID » : « 4c9a97db0000000f13ee4446b9aead9b »,<br>            « taskNumber » : 1 <br>        },<br>        « 4ca28ba600002024cd49e75bd43cf601 » : {<br>            « percentComplete » : 0,<br>            « status »: « INP:A »,<br>            « name » : « seconde tâche »,<br>            « ID » : « 4ca28ba600002024cd49e75bd43cf601 »,<br>            « taskNumber » : 2 <br>        } <br>    } <br>}</pre>
...
<pre>/attask/api/v15.0/task/search?fields=scheduledStartDate,priority</pre>renvoie une réponse similaire à ce qui suit :
<pre>{<br>    « priorité » : 2,<br>    « name » : « première tâche »,<br>    « ID » : « 4c7c08fa0000002ff924e298ee148df4 »,<br>    « scheduledStartDate » : « 2010-08-30T09:00:00:000-0600 » <br>}</pre>

>[!NOTE]
>
>Ces noms de champ respectent la casse.

Pour obtenir la liste des références de champ possibles, voir  [Explorateur API](../../wf-api/general/api-explorer.md)

#### Recherche d’objets imbriqués

Vous pouvez rechercher des objets imbriqués. Par défaut, les objets imbriqués sont renvoyés avec uniquement le nom et l’ID. Par exemple, pour obtenir tous les événements avec leurs propriétaires, utilisez la requête suivante :
<pre>/attask/api/v15.0/issue/search?fields=owner</pre>Si des informations supplémentaires sont nécessaires, vous pouvez demander un champ imbriqué en utilisant la syntaxe des deux-points. Par exemple, la requête suivante recherche tous les événements avec le nom, l’ID, le titre et le numéro de téléphone du propriétaire
<pre>/attask/api/v15.0/issue/search?fields=owner:title,owner:phoneNumber</pre>et renvoie ce qui suit : 
<pre>{<br>    « name » : « un sujet important »,<br>    « ID » : « 4c78285f00000908ea8cfd66e084939f »,<br>    « owner » : {<br>        « title » : « Spécialiste des opérations »,<br>        « phoneNumber » : « 555-1234 »,<br>        « name » : « Utilisateur administrateur »,<br>        « ID » : « 4c76ed7a0000054c172b2c2d9f7f81c3 » <br>    } <br>}</pre>

#### Récupération de collections imbriquées
...
Pour définir un projet afin qu’il soit partagé uniquement avec un utilisateur disposant de l’ID « abc123 », utilisez la requête suivante :
...
Les mises à jour des objets sont toujours effectuées par ID à l’aide de l’URI unique de l’objet . Les champs à mettre à jour sont spécifiés comme paramètres de requête. Par exemple, pour modifier le nom d’un projet, vous pouvez envoyer une demande similaire à celle-ci :
...
Comme le montre l’exemple suivant, vous pouvez utiliser le paramètre de requête de mises à jour pour spécifier les champs à mettre à jour à l’aide de la syntaxe JSON :
<pre>PUT /attask/api/v15.0/project/4c7... ?mises à jour= <br>{<br>     nom : « Nouveau nom du projet », <br>     statut : « CUR », <br>     ... <br>}</pre>
...
<pre>PUT /attask/api/v15.0/task/4c7... ?mises à jour= <br>{<br>    affectations : [ <br>        { <br>            assignedToID : « 2222...54d0, <br>            assignmentPercent : 50.0 <br>        },{ <br>            roleID : « 1111...54d0 »<br>        } <br>    ] <br>}</pre>

>[!NOTE]
>
>Bien que les mises à jour apportées au niveau supérieur soient éparses, les mises à jour apportées à une collection ou à un objet imbriqué remplacent complètement la collection existante. Pour modifier une affectation unique sur une tâche sans affecter les objets, utilisez le PUT sur l’affectation plutôt que sur la tâche.

L&#39;exemple suivant transforme un projet en file d&#39;attente de Help Desk publique. Notez que les propriétés de file d’attente existantes sont remplacées.
<pre>PUT /attask/api/v15.0/project/4c7... ?mises à jour= <br>{ <br>    queueDef : { <br>        isPublic : 1 <br>    } <br>}</pre>

### Utilisation du paramètre de demande d&#39;action

Certains objets prennent en charge des actions supplémentaires qui peuvent être effectuées en plus des modifications simples. Vous pouvez spécifier ces actions à l’aide du paramètre de demande d’action. Par exemple, la requête suivante recalcule la chronologie d’un projet donné :
...
L’exemple suivant illustre la syntaxe permettant de déplacer une tâche d’un projet à un autre :
<pre>PUT /attask/api/v15.0/task/4c7.../move?projectID=5d8...</pre>Un exemple pour chaque type d’action est fourni ici : (??)
<pre>PUT /attask/api/v15.0/project/1234/approveApproval<br><br>PUT /attask/api/v15.0/project/1234/calculateFinance<br><br>PUT /attask/api/v15.0/project/1234/calculateTimeline<br><br>PUT /attask/api/v15.0/project/1234/calculateDataExtension<br><br>PUT /attask/api/v15.0/project/1234/reminderApproval<br><br>PUT /attask/api/v15.0/project/1234/rejetApproval<br><br>PUT /attask/api/v15.0/task/1234/move<br><br>PUT /attask/api/v15.0/workitem/1234/markViewed</pre>Seule l’action de déplacement nécessite l’identification d’attributs supplémentaires pour spécifier le projet dans lequel l’élément de travail doit être déplacé.
...
L’exemple suivant illustre la syntaxe de partage d’un projet avec une équipe :
<pre>PUT /attask/api/v15.0/project/123abcxxxxxxxxxxxxxxxxxxxxxxxxxxxx/share?accessorID=123abcxxxxxxxxxxxxxxxxxxxxxxxxxxxx&amp;accessorObjCode=TEAMOB</pre>Lors de la modification d’un objet, vous pouvez remplacer toutes les règles d’accès sur un objet en effectuant une opération de PUT et en envoyant des mises à jour similaires à l’exemple suivant :
<pre>PUT /attask/api/v15.0/project/123abcxxxxxxxxxxxxxxxxxxxxxxxx?method=PUT&amp;updates={accessRules:[{accessorID:'123abcxxxxxxxxxxxxxxxxxxxxxxxxxxxx',accessorObjCode:'TEAMOB',coreAction:'VIEW'}]}</pre>L'exemple suivant illustre la syntaxe permettant de déplacer une tâche d'un projet à un autre :
<pre>PUT /attask/api/v15.0/task/4c7.../move?projectID=5d8...</pre>

## Comportement du DELETE

DELETE supprime un objet. Dans tous les cas, l’URI peut inclure le paramètre force=true pour que le serveur supprime les données spécifiées et leurs dépendances. Dans l’exemple ci-dessous, une tâche est supprimée en exécutant la méthode de DELETE HTTP sur un URI :
...
Une instruction de mise à jour en bloc met à jour plusieurs objets simultanément au sein d’un seul appel API. Un appel API de création en bloc est créé de la même manière qu’un appel de mise à jour normal, comme illustré dans les exemples suivants :
<pre>PUT /attask/api/v15.0/proj?updates=[{« name »:« Test_Project_1 »},{« name »:« Test_Project_2 »}]&amp;method=POST&amp;apiKey=123ab-cxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx</pre>qui génère un retour similaire à ce qui suit :
<pre>data : [{<br>    ID : « 53ff8d3d003b438b57a8a784df38f6b3 »,<br>    nom : « Test_Project_1 »,<br>    objCode : « PROJ »,<br>    percentComplete : 0,<br>    Date d'achèvement prévue : « 2014-08-28T11:00:00:000-0400 »,<br>    scheduledStartDate : « 2014-08-28T11:00:00:000-0400 »,<br>    priorité : 0,<br>    expectedCompletionDate : « 2014-08-28T16:12:00:000-0400 »,<br>    Statut : « CUR »<br>},<br>{<br>    ID : « 53ff8d49003b43a2562aa34eea3b6b10 »,<br>    nom : « Test_Project_2 »,<br>    objCode : « PROJ »,<br>    percentComplete : 0usi,<br>    Date d'achèvement prévue : « 2014-08-28T11:00:00:000-0400 »,<br>    scheduledStartDate : « 2014-08-28T11:00:00:000-0400 »,<br>    priorité : 0,<br>    expectedCompletionDate : « 2014-08-28T16:12:00:000-0400 »,<br>    Statut : « CUR »<br>}]</pre>Vous pouvez également effectuer une mise à jour en bloc semblable à ce qui suit :
<pre>PUT /attask/api/v15.0/proj?Umethod=PUT&amp;updates=[{« ID »:« 123abcxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx »,« name »:« Test_Project_1_ Edit »},{« ID »:« 123abcxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx »,« name »:« Test_Project_2_Edit »}]&amp;apiKey=123abcxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx</pre>qui génère un retour similaire à ce qui suit :
<pre>data : [ {<br>     ID : « 53ff8e15003b461d4560f7f65a440078 »,<br>     nom : « Test_Project_1_Edit »,<br>     objCode : « PROJ »,<br>     percentComplete : 0,<br>     Date d'achèvement prévue : « 2014-08-28T11:00:00:000-0400 »,<br>     scheduledStartDate : « 2014-08-28T11:00:00:000-0400 »,<br>     priorité : 0,<br>     expectedCompletionDate : « 2014-08-28T16:16:00:000-0400 »,<br>     Statut : « CUR »<br>},<br>{<br>    ID : « 53ff8e19003b46238a58d303608de502 »,<br>    nom : « Test_Project_2_Edit »,<br>    objCode : « PROJ »,<br>    percentComplete : 0,<br>    Date d'achèvement prévue : « 2014-08-28T11:00:00:000-0400 »,<br>    scheduledStartDate : « 2014-08-28T11:00:00:000-0400 »,<br>    priorité : 0,<br>    expectedCompletionDate : « 2014-08-28T16:16:00:000-0400 »,<br>    Statut : « CUR »<br>}]</pre>Si vous souhaitez que toutes les opérations se produisent dans la même transaction, ajoutez « atomic=true » à votre appel API par lot en tant que paramètre de requête. Ainsi, si l’une des opérations échoue, toutes les opérations sont annulées.
