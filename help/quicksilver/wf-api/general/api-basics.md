...
<pre>{<br>    "priority": 2,<br>    "name": "first task",<br>    "ID" : "4c7c08fa0000002ff924e298ee148df4",<br>    "scheduledStartDate": "2010-08-30T09:00:00:000-0600" <br>}</pre>
...
<pre>{<br>    "name" : "une question importante",<br>    "ID" : "4c78285f00000908ea8cfd66e084939f",<br>    "owner": {<br>        "title" : "spécialiste des opérations",<br>        "phoneNumber": "555-1234",<br>        "name": "Admin User",<br>        "ID" : "4c76ed7a0000054c172b2c2d9f7f81c3" <br>    } <br>}</pre>
...
<pre>{<br>    "First Project": { <br>        "sum_hours": 15 <br>    }, <br>     "Second Project": { <br>        "sum_hours": 30 <br>    } <br>}</pre>L'ajout du paramètre $$ROLLUP=true inclut un total à chaque niveau de groupement :
...
<pre>GET /attask/api/v15.0/project/123abcxxxxxxxxxxxxxxxxxxxxxxxx?method=put &amp;loads={ accessRules: [ {accessorID: 'abc123', accessorObjCode: 'USER', coreAction: 'VIEW'} ] } }</pre>Vous pouvez également effectuer un partage uniquement avec une nouvelle personne et conserver les autorisations existantes intactes :
...
<pre>POST /attask/api/v15.0/document?update={<br>    name: aFileName,<br>    handle : abc...123, (handle à partir du téléchargement du fichier)<br>    docObjCode : PROJ (ou TASK, OPTASK, etc.)<br>    objID : abc...123,<br>    currentVersion:{version:v1.0,fileName:aFileName}<br>}</pre>
...
<pre>PUT /attask/api/v15.0/project/4c7...?update= <br>{<br>     name: "New Project Name", <br>     status : "CUR", <br>     .. <br>}</pre>
...
<pre>PUT /attask/api/v15.0/task/4c7...?update= <br>{<br>    assignments : [ <br>        { <br>            assignedToID : "222...54d0, <br>            assignmentPercent : 50.0 <br>        },{ <br>            roleID: "1111...54d0"<br>        } <br>    ] <br>}</pre>
...
<pre>PUT /atask/api/v15.0/project/1234?method=put&amp;updated={accessRules:[{accessorID: 'abc123', accessorObjCode: 'USER', coreAction: 'VIEW'}]}</pre>
