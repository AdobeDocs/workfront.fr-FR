...
1. Remplacez l’URL par /attask/api/v9.0/project/search.\
...
Notez que la requête /attask/api/v9.0/project?id=... est identique à la requête `/attask/api/v9.0/project/...` requête.
...
<pre>GET /attask/api/v9.0/task/search?name=Planning<br>&amp;name_Mod=contains<br>&amp;OR:1:portfolio:name=FixedAssets<br>&amp;OR:1:portfolio:name_Mod=eq<br>&amp;OR:1:assignedTo:name=Steve<br>&amp;OR:1:assignedTo:name_Mod=contains<br>&amp;OR:2:parent:name=Final Task<br>&amp;OR:2:parent:name_Mod=eq
</pre>
...
<pre>/atask/api/v9.0/user/search?username=testuser@workfront.com</pre>transmettez le paramètre d’URL dans un filtre, comme illustré dans l’exemple suivant :
<pre>/atask/api/v9.0/user/search?filters={"username":"testuser@workfront.com"}</pre>
...
<pre>/attask/api/v9.0/task/search?map=true</pre>renvoie une réponse indexée par un identifiant similaire à ce qui suit :
...
<pre>/task/api/v9.0/task/search?fields=scheduledStartDate,priority</pre>renvoie une réponse similaire à celle-ci :
...
<pre>/atask/api/v9.0/project/search?fields=tasks</pre>La requête suivante récupère des affectations de tâche :
...
<pre>/attask/api/v9.0/project/search?fields=DE:CustomText</pre>qui renverra
...
<pre>/atask/api/v9.0/project/search?fields=parameterValues</pre>renvoie des données similaires aux suivantes :
...
<pre>GET /attask/api/v9.0/project/count?status=CUR</pre>renvoie le nombre de résultats au format suivant :
...
<pre>GET /attask/api/v9.0/hour/report?project:name_1_GroupBy=true&amp;hours_AggFunc=sum</pre>qui renvoie le résultat suivant
...
<pre>GET /attask/api/v9.0/project/123abcxxxxxxxxxxxxxxxxxxxxxxxxxx?method=put &amp;updated={ accessRules: [ {accessID: 'abc123', accesseurObjCode: 'USER', coreAction: 'VIEW'} ] }</pre>Vous pouvez également effectuer un partage uniquement avec une nouvelle personne et conserver les autorisations existantes intactes :
<pre>GET /attask/api/v9.0/project/123abcxxxxxxxxxxxxxxxxxxxxxxxxxx/share?method=put&amp;accessorID=abc123&amp;accessorObjCode=USER&amp;coreAction=VIEW</pre>Pour récupérer les règles d’accès existantes :
...
<pre>POST /attask/api/v9.0/project?name=New Project</pre>La réponse inclut le projet nouvellement créé, ainsi que son nouvel identifiant et tous les autres champs spécifiés.
...
<pre>POST /attask/api/v9.0/upload</pre>L’API exige que le type de contenu soit multipart/form-data. Le nom du paramètre du fichier doit être uploadedFile. Le serveur renvoie les données JSON suivantes :
...
<pre>POST /attask/api/v9.0/document?update={<br>    name: aFileName,<br>    handle : abc...123, (gérer à partir du téléchargement du fichier)<br>    docObjCode : PROJ (ou TÂCHE, OPTASK, etc.)<br>    objID : abc...123,<br>    currentVersion:{version:v1.0,fileName:aFileName}<br>}</pre>
...
<pre>PUT /attask/api/v9.0/project/4c7...?name=New Project Name <br>PUT /attask/api/v9.0/project?id=4c7..&amp;name=New Project Name</pre>Comme la mise à jour nécessite un identifiant, cette opération échoue (sans insertion) si l’objet n’existe pas sur le serveur.
...
<pre>PUT /attask/api/v9.0/project/4c7...?update= <br>{<br>     name: "Nouveau nom du projet", <br>     status : "CUR", <br>     ... <br>}</pre>
...
<pre>PUT /attask/api/v9.0/task/4c7...?update= <br>{<br>    assignments : [ <br>        { <br>            assignedToID: "2222...54d0, <br>            assignmentPercent : 50,0 <br>        },{ <br>            roleID : "1111...54d0"<br>        } <br>    ] <br>}</pre>
...
<pre>PUT /attask/api/v9.0/task/4c7.../move?projectID=5d8...</pre>Voici un exemple pour chaque type d’action : (??)
<pre>PUT /attask/api/v9.0/project/1234/approveApproval<br><br>PUT /attask/api/v9.0/project/1234/calculateFinance<br><br>PUT /attask/api/v9.0/project/1234/calculateTimeline<br><br>PUT /attask/api/v9.0/project/1234/calculateDataExtension<br><br>PUT /attask/api/v9.0/project/1234/rappelApproval<br><br>PUT /attask/api/v9.0/project/1234/rejectApproval<br><br>PUT /attask/api/v9.0/task/1234/move<br><br>PUT /attask/api/v9.0/workitem/1234/markViewed</pre>Seule l’action de déplacement nécessite l’identification d’attributs supplémentaires pour spécifier le projet dans lequel l’élément de travail doit être déplacé.

Voici un exemple de chaque type d’action : 
<pre>PUT /attask/api/v9.0/project/1234?method=put&amp;updated={accessRules:[{accessorID: 'abc123', accesseurObjCode: 'USER', coreAction: 'VIEW'}]}</pre>

### Partage d’objets

L’exemple suivant illustre la syntaxe du partage d’un projet avec une équipe :
<pre>PUT /attask/api/v9.0/project/123abcxxxxxxxxxxxxxxxxxxxxxxxxxxxx/share?accessorID=123abcxxxxxxxxxxxxxxxxxxxxxxxxxx&amp;accessorObjCode=TEAMOB</pre>Lors de la modification d’un objet, vous pouvez remplacer toutes les règles d’accès d’un objet en effectuant un PUT et en envoyant des mises à jour similaires à l’exemple suivant :
<pre>PUT /attask/api/v9.0/project/123abcxxxxxxxxxxxxxxxxxxxxxxxxxx?method=PUT&amp;update={accessRules:[{accessorID:'123abcxxxxxxxxxxxxxxxxxxxxxxxxxx',accessorObjCode:'TEAMOB',coreAction: W'}]}</pre>L’exemple suivant illustre la syntaxe du déplacement d’une tâche d’un projet à un autre :
...
<pre>PUT /attask/api/v9.0/proj?updated=[{"name":"Test_Project_1"},{"name":"Test_Project_2"}]&amp;method=POST&amp;apiKey=123ab-cxxxxxxxxxxxxxxxxxxxxxxxxxxxx</pre>qui génère un retour similaire à ce qui suit :
...
<pre>PUT /attask/api/v9.0/proj?Umethod=PUT&amp;update=[{"ID":"123abcxxxxxxxxxxxxxxxxxxxxxx","name":"Test_Project_1_ Edit"},{"ID":"123abcxxxxxxxxxxxxxxxxxxxx","name": "Test_Project_2_Edit"}]&amp;apiKey=123abcxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx</pre>qui génère un retour similaire à ce qui suit :
