...
description: Les tâches planifiées sont représentées dans Adobe Workfront par des projets et des tâches. Cependant, vous pouvez travailler dans un environnement où le travail non planifié, sous la forme de requêtes aléatoires, peut intervenir à tout moment. Workfront fournit un workflow pour s’adapter à ce type d’environnement en utilisant les files d’attente de demande.
author: Alina
...
Pour plus d’informations sur la conversion de problèmes en tâche ou projet, consultez l’article . [Présentation des problèmes de conversion dans Adobe Workfront](../../../manage-work/issues/convert-issues/convert-issues.md).
...
En tant qu’administrateur de Workfront, vous devez créer des files d’attente de demande et les mettre à la disposition des utilisateurs avant qu’ils puissent utiliser cette fonctionnalité. Un utilisateur disposant d’une licence de planificateur et disposant des autorisations Modifier les projets et Gérer pour un projet spécifique peut également créer des files d’attente de demande. 

Pour plus d’informations sur la création de files d’attente de requêtes, reportez-vous à l’article . [Création d’une file d’attente de requête](../../../manage-work/requests/create-and-manage-request-queues/create-request-queue.md).

Vous devez créer les composants suivants d’une file d’attente de requête :
...
1. (Conditionnel) Cliquez dans le **Type de requête** et effectuez l’une des opérations suivantes :
   ...
   * Dans la **Requêtes** , sélectionnez une file d’attente de requêtes.
   * Saisissez un mot-clé appartenant à un chemin d’accès précédemment utilisé pour rechercher une file d’attente de requêtes.
   ...
   >[!TIP]
   >
   >* Les 200 premières files d’attente de requêtes s’affichent par défaut, dans l’ordre alphabétique.
   ...
   >   
   >Pour plus d’informations sur la publication d’un projet en tant que file d’attente des demandes d’aide, reportez-vous à l’article . [Création d’une file d’attente de requête](../../../manage-work/requests/create-and-manage-request-queues/create-request-queue.md).
...
1. (Facultatif) Si la file d’attente des demandes comprend des groupes de rubriques, sélectionnez le nom du groupe de rubriques dans le premier champ déroulant. Sinon, sélectionnez une rubrique de file d’attente.
   ...
   Pour plus d’informations sur la création de groupes de rubriques, reportez-vous à l’article . [Création de groupes de rubriques](../../../manage-work/requests/create-and-manage-request-queues/create-topic-groups.md). Pour plus d’informations sur la création de rubriques de file d’attente, voir l’article [Création de rubriques de file d’attente](../../../manage-work/requests/create-and-manage-request-queues/create-queue-topics.md).
...
1. Selon les champs activés par l’administrateur Workfront dans la variable **Nouveaux champs de problème** de la section **Détails de la file** sous-onglet du projet, vous pouvez trouver l’un des champs suivants lorsque vous envoyez une nouvelle demande :
   ...
   <table style="table-layout:auto"> 
    ...
    <tbody> 
     <tr> 
      ...
      <td>Indiquez un nom pour votre requête. Il s’agit d’un champ obligatoire.</td> 
     </tr> 
     <tr> 
     ...
     <tr data-mc-conditions="QuicksilverOrClassic.Quicksilver"> 
      ...
      <td> <p><span>Indiquez le nom d’un utilisateur principal, d’un rôle de tâche ou d’une équipe à laquelle la requête doit être affectée.</span> </p> <p>Vous ne pouvez spécifier qu’une seule équipe.</p>
   ...
   <tr> 
     ...
     <tr> 
      ...
      <td>L’état par défaut d’une nouvelle requête est "Nouveau". Votre administrateur système a peut-être modifié le nom de cet état. Vous pouvez également modifier l’état pour autre chose à partir de ce menu déroulant.</td> 
     </tr> 
     <tr> 
...
1. Utilisez l’une des méthodes suivantes :
   ...
   * Cliquez sur **Envoyer** si vous êtes prêt à envoyer la demande. La demande est enregistrée dans la section Envoyé . Selon la règle de routage de la file d’attente des requêtes, cette requête peut être acheminée vers un projet différent de celui désigné comme file d’attente des requêtes. Pour plus d’informations sur les règles de routage, voir [Création de règles de routage](../../../manage-work/requests/create-and-manage-request-queues/create-routing-rules.md).
   ...
   Pour plus d’informations sur le traitement des requêtes entrantes, reportez-vous à l’article [Gérer les requêtes de travail et d’équipe](../../../people-teams-and-groups/work-with-team-requests/manage-work-and-team-requests.md).
...
Vous pouvez partager un lien direct vers une file d’attente de requêtes lorsque vous envoyez une nouvelle requête et l’incorporez dans d’autres applications. Les utilisateurs qui accèdent à ce lien à partir du Web ou d’autres applications doivent également être connectés avec un compte Workfront principal pour pouvoir accéder à cette file d’attente et lui envoyer des requêtes. Pour plus d’informations, voir [Partage d’un lien vers une file d’attente de requêtes](../../../manage-work/requests/create-requests/share-link-to-request-queue.md).
...
Pour plus d’informations sur l’envoi de requêtes par le biais de l’application mobile, voir [Demandes](../../../workfront-basics/mobile-apps/using-the-workfront-mobile-app/workfront-for-android.md#requests) dans les articles :
