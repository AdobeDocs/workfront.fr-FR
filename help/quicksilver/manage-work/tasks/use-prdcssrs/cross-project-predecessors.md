...
Comme les prédécesseurs d’un seul projet, Adobe Workfront permet aux tâches d’être dépendantes des tâches d’autres projets.

**EXEMPLE**

Si une entreprise d’excavation ne dispose que d’une seule arrière-plan et que deux projets simultanés ont des tâches qui nécessitent l’utilisation de la arrière-plan, le chef de projet peut rendre la tâche du premier projet dépendante de la tâche du deuxième projet afin d’illustrer que l’excavation peut commencer lorsque le projet précédent abandonnera la arrière-plan.
Lors de la liaison de projets par le biais de prédécesseurs interprojets, les dates du Principal projet (celui qui a la tâche précédente) auront un impact sur le projet secondaire (celui qui a la tâche qui lui succède).
...
1. Accédez à la tâche qui sera votre successeur.
1. Cliquez sur **Prédécesseurs** dans le panneau de gauche.
1. Cliquez sur **Ajouter un prédécesseur.**
1. Dans le **Projet parent** , commencez à saisir le nom du projet qui contient la tâche dont vous souhaitez qu’elle dépend de votre tâche actuelle.
1. Cliquez sur le nom qui apparaît dans la liste déroulante.
1. Dans le **Tâches** , commencez à saisir le nom de la tâche que vous souhaitez dépendre de la tâche en cours.
1. Indiquez les informations suivantes pour définir la relation entre le prédécesseur et la tâche dépendante :

   * **Type de dépendance :** Sélectionnez la relation que vous souhaitez que la tâche ait avec la tâche dépendante. La relation par défaut est &quot;Finish-Start&quot;, ce qui signifie que la tâche du prédécesseur doit se terminer avant que la tâche dépendante puisse commencer. Pour plus d’informations sur les différents types de dépendances, voir [Présentation des types de dépendances des tâches](../../../manage-work/tasks/use-prdcssrs/task-dependency-types.md)
...
1. Cliquer sur **Enregistrer**.
   ...
   Les tâches qui ont un prédécesseur multi-projet affichent le numéro de référence du projet auquel appartient le prédécesseur et le numéro de la tâche, séparés par un deux-points dans la colonne Prédécesseurs d’une liste de tâches.
   ...
   Passez la souris sur cette valeur pour obtenir plus d’informations sur le prédécesseur, le projet et les dates. Cliquez sur le prédécesseur de plusieurs projets dans la zone Détails pour ouvrir la tâche. Cliquez sur **Voir Projet** pour ouvrir le projet croisé.

   ![Détails des prédécesseurs sur plusieurs projets](assets/cross-project-predecessor-details.png)

   >[!TIP]
   >
   >   Le **Voir Projet** s’affiche uniquement lors de l’affichage d’un prédécesseur multi-projet.
