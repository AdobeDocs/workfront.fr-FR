...
* **Application d’un prédécesseur :** Lors de l’application d’un prédécesseur, la tâche qui lui succède ne peut pas commencer tant que le prédécesseur n’est pas terminé. La tâche de remplacement s’affiche comme commençant immédiatement une fois le prédécesseur terminé.
...
* **Balises :** Vous pouvez créer des décalages dans vos dépendances, ce qui crée un délai qui doit survenir après l’achèvement d’une tâche de prédécesseur et avant que la tâche qui lui succède puisse commencer. Les décalages affectent la chronologie du projet.
...
* Accédez au projet sur lequel vous travaillez et procédez comme suit :
   ...
   1. Cliquez sur le bouton **Tâches** .
   1. Choisissez la **Vue standard** en haut de la liste des tâches.
   1. Le **Prédécesseurs** affiche les numéros des tâches du prédécesseur.
...
* **4515:2** Le numéro de tâche du prédécesseur est 2. - Il s’agit d’une dépendance Terminer au début non appliquée avec le prédécesseur du projet avec le numéro de référence. **4515**; le numéro de la tâche précédente est **2**.
