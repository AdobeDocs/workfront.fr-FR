...
1. (Facultatif) Pour ajouter des articles à une itération existante, voir [Ajouter des articles à une itération existante](../../../agile/use-scrum-in-an-agile-team/iterations/add-stories-to-existing-iteration.md).
...
Lorsque vous ajoutez une tâche en tant qu’article à une itération, la fonction [!UICONTROL Doit se terminer sur la tâche] La contrainte est utilisée pour chaque histoire. Dans la plupart des cas, la date de début prévue de la tâche est calculée selon la formule suivante :

[!UICONTROL Date de fin de l’itération] moins (-) [!UICONTROL Durée de la tâche] est égal à (=) [!UICONTROL Date de début planifiée de la tâche]

La variable [!UICONTROL Date de fin du projet] est utilisé à la place de si la date de début du projet est postérieure à la date de début de l’itération et que la date de fin du projet est postérieure à la date de fin de l’itération.
