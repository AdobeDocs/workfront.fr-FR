...
title: Afficher l’état et l’état d’avancement d’un BAT dans [!DNL Workfront Proof]
...
# Afficher l’état et l’état d’avancement d’un BAT dans [!DNL Workfront Proof]
...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td> <p><strong>Envoyé</strong>. Le BAT a été envoyé aux validants.</p> </td> 
  </tr> 
  <tr> 
   <td> <p> <img src="assets/proof-progress-opened-icon.png" alt="proof_progress_opened_icon.png"> </p> <p> </p> </td> 
   <td> <p><strong>Ouvert</strong>. Un réviseur a ouvert la page Détails du BAT ou a ouvert le BAT lui-même dans la visionneuse de BAT.</p> </td> 
  </tr> 
  <tr> 
   <td> <p> <img src="assets/proof-progress-comment-icon.png" alt="proof_progress_comment_icon.png"> </p> </td> 
   <td> <p><strong>Commentaires</strong>. Les validants (utilisateurs qui peuvent faire des commentaires) ont fait des commentaires sur le BAT.</p> <p>Si aucun validant n'est désigné pour le BAT, cette icône ne s'affiche pas.</p> </td> 
  </tr> 
  <tr> 
...
Workfront BAT utilise les icônes de progression pour suivre la progression d’un BAT à chacun des niveaux suivants :
...
Pour un exemple de la méthode [!DNL Workfront Proof] détermine la progression à l’aide de l’opérateur validant ou de l’étape qui est le plus en retard, supposons que trois opérateurs validants sur un BAT doivent prendre une décision. Si deux d&#39;entre eux ont pris leur décision mais que le troisième ne l&#39;a pas fait, la barre de progression du BAT n&#39;affiche pas le D en vert en raison de la décision en suspens.

Si la variable [!UICONTROL Principal décisionnaire] est sélectionné sur un BAT et que le décideur Principal envoie une décision, le D dans la barre de progression du BAT devient vert pour tous les réviseurs, car aucune autre décision n’est requise.

De même, si la variable [!UICONTROL Une seule décision requise] est sélectionné sur un BAT et tout validant envoie une décision, le D dans la barre de progression du BAT devient vert pour tous les validants, car aucune autre décision n’est requise.
...
Le statut du BAT dépend du participant au &quot;pire cas&quot;. Par exemple, supposons qu’il y ait trois décisions sur le BAT : deux ont le statut de **Acceptée** et l’un a le statut de **Rejetés**. La décision &quot;pire cas&quot; du Refusé annule les autres décisions et le statut général du BAT est indiqué comme **Rejetés**.
...
![Summary_extenable.png](assets/summary-expandable-350x68.png)
...
Le [!UICONTROL Actions de l’environnement] s’affiche lorsque vous passez la souris sur la section d’évaluation (1) et vous permet de
...
* Modifier les détails du réviseur (3) : permet de modifier le nom d’affichage, le rôle du BAT et l’alerte par e-mail de ce réviseur.
* Faire d&#39;eux le propriétaire du BAT (4)
...
Chaque BAT comporte également un menu (1) qui vous permet d’effectuer les actions suivantes :
...
Pour plus d’informations sur l’affichage de l’état et de l’état du BAT dans [!DNL Workfront], voir [Progression et état de l’affichage](#viewing-progress-and-status).

Pour plus d’informations sur l’affichage de la progression et de l’état dans la visionneuse de vérification de l’appli de bureau, voir [Vérification d’un workflow dans la visionneuse de vérification](../../../workfront-proof/wp-work-proofsfiles/review-proofs-wpv/review-workflow.md).
