---
title: Création D’Objets À L’Aide Des Automatisations D’Enregistrements De Planification D’Adobe Workfront
description: Après avoir configuré et activé les automatisations dans Adobe Workfront Planning, vous pouvez les utiliser pour créer des objets dans Adobe Workfront ou des enregistrements dans Adobe Workfront Planning. Cet article décrit comment créer des enregistrements ou des objets à l’aide d’une automatisation existante.
...
Par exemple, vous pouvez utiliser une automatisation existante qui prend une campagne Workfront Planning et crée un projet dans Workfront pour suivre la progression de cette campagne. Le projet serait connecté à la campagne de planification Workfront dans le champ Projet connecté de la campagne.

Cet article décrit comment créer des enregistrements Workfront Planning ou des objets Workfront à l’aide d’une automatisation existante.

Pour plus d’informations sur la création d’automatisations pour un type d’enregistrement, voir [Configurer les automatisations d’Adobe Workfront Planning pour créer des enregistrements](/help/quicksilver/planning/records/configure-automations-to-create-records.md).

Une fois que vous avez créé des enregistrements ou des objets Workfront à l’aide des automatisations, ils sont automatiquement liés aux enregistrements à partir desquels vous exécutez l’automatisation.
...
* Un enregistrement Workfront Planning
...
<tr> 
   ...
   <p>Gérez les autorisations de l’enregistrement pour lequel vous exécutez l’automatisation.</p>
...
* Le nom de l&#39;objet ou de l&#39;enregistrement créé par une automatisation est identique au nom d&#39;enregistrement à partir duquel vous le créez, lorsque vous créez un seul objet.

* Lorsque vous créez plusieurs projets, ils sont automatiquement nommés selon le modèle suivant :

  `[ Name of the record ] Name of the field choice`

  Pour plus d’informations, reportez-vous à la section [Utiliser une automatisation Workfront Planning pour créer un objet ou un enregistrement](#use-a-workfront-planning-automation-to-create-an-object-or-a-record) de cet article.
...
* Lorsque votre entreprise utilise l’espace de stockage cloud Workfront et Adobe hérité, les éléments créés automatiquement à partir de Planning sont créés à l’aide de l’espace de stockage par défaut sélectionné par l’administrateur Workfront dans la zone Configuration.
...
1. Cliquez sur le bouton d’automatisation sur la barre bleue.
   ...
   * Le nouvel objet s’affiche dans le champ connecté indiqué dans la configuration du bouton d’automatisation. Vous devrez peut-être actualiser votre page avant d’afficher le nouvel objet. Le nouvel objet porte le même nom que l’enregistrement d’origine.

   * Si plusieurs projets ont été créés en fonction du choix de champs à sélection multiple ou unique, les projets sont automatiquement nommés selon le modèle suivant :
