...
L’obstacle le plus courant à un déploiement réussi n’est pas la technologie. C&#39;est « la faute à la technologie ». Lorsque les équipes rencontrent le défi de travailler avec de nouvelles personnes, de nouveaux processus et de nouveaux outils à la fois, l’outil devient une cible de frustration.
...
Pour activer ce changement d’évolution, procédez comme suit :

1. Choisissez une équipe pilote prête et disposée à innover.
2. Utilisez les 15 premiers jours de votre lancement pour prouver le modèle par l’un des deux chemins à grande vitesse :

   * **Chemin parallèle (ou approche de double suivi) :** l’équipe conserve son workflow actuel tout en modélisant simultanément son prochain cycle de planification dans Workfront Planning.

     Cela renforce la mémoire musculaire et la confiance sans le risque d&#39;un point unique de défaillance.

   * **La rupture nette :** après un exercice de conception initial à l’aide de données d’exemple pour valider le modèle, l’équipe déplace son point de départ réel directement dans Workfront Planning pour son cycle suivant.
...
Nous vous recommandons de rechercher constamment des marqueurs de succès et de les ajuster en fonction de vos résultats.
