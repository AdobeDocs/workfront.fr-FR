...
Le [!DNL Adobe Unified Experience] pour [!DNL Workfront] vous permet de gérer l’ensemble de vos [!DNL Adobe] à un seul emplacement avec une seule connexion. Le [!DNL Adobe] la zone de navigation est intégrée de manière transparente à [!DNL Workfront]. Certaines fonctionnalités sont différentes et vous pouvez poser quelques questions en tant que [!DNL Workfront] est migrée vers l’expérience unifiée.
...
| Une seule URL de connexion est disponible pour tous les [!DNL Adobe Experience Cloud] applications | Connectez-vous à [!DNL Workfront] avec un [!DNL Workfront] URL |
| Un &quot;sélecteur d’organisation&quot; vous permet de passer de [!DNL Workfront] organisations et environnements | Le &quot;sélecteur d’organisation&quot; n’est pas disponible. |
...
**Comment puis-je en savoir plus sur le [!DNL Adobe Admin Console]?**
...
Les clients existants seront contactés pour planifier des migrations. Les collègues de l’équipe d’assistance à la migration guident les clients tout au long du processus, donnent des conseils sur [!DNL Admin Console] et fournissez des liens vers la documentation nécessaire pour que le déplacement soit aussi simple et facile que possible.
...
**Comment cela affecte-t-il les clients qui disposent déjà du panneau d’administration d’AEM pour [!DNL Adobe Assets] - mais l’authentification unique est configurée différemment de[!DNL Workfront?]**
...
Utilisateurs n’ayant pas reçu l’accès à [!DNL Adobe Admin Console] pour entrer dans [!DNL Workfront] doit créer un[!UICONTROL compte personnel]&quot; ou [!DNL Adobe] ID pour pouvoir se connecter. Cette action envoie un courrier électronique à l’administrateur pour approuver ou rejeter sa demande. Elle permet en outre à l’administrateur de configurer le type d’accès dont dispose cet utilisateur. Lorsqu’ils se connectent, ils accèdent à experience.adobe.com, saisissent leur adresse électronique et choisissent [!UICONTROL Compte personnel]. A partir de là, ils pourront accéder à [!DNL Workfront].
...
**Nous avons des utilisateurs externes inclus dans notre [!DNL Workfront] instance. Nous ne voudrions pas qu’ils aient accès à d’autres produits inclus dans [!DNL Adobe]. Comment limiter leur accès dans la console ?**
