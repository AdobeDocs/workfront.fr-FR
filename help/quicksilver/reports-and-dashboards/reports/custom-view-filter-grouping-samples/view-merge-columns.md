...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td> <p> Actuel : 
   <ul>
   <li>Demander la modification d’une vue</li> 
   <li>Prévoir la modification d’un rapport</li>
   </ul>
     </p>
     <p> Nouveau : 
   <ul>
...
* La ligne `valueformat=HTML` est obligatoire dans une colonne partagée. Dans le cas contraire, les colonnes ne contiennent aucune information (elles seront vides) lorsque le rapport est exporté depuis Adobe Workfront.
...
* Les colonnes avec l’attribut **viewalias** peuvent limiter la quantité de colonnes que vous pouvez fusionner. Pour éviter ces limites, évitez d’utiliser l’attribut **viewalias** . Si vous devez inclure l’attribut **viewalias** dans une colonne, assurez-vous qu’il s’agit du dernier élément répertorié dans la colonne.
...
* Lorsque l’une des colonnes ou les deux affichent un champ de type `tile`, un saut de ligne forcé est automatiquement inséré dans la colonne fusionnée. Par exemple, les champs de type Texte avec mise en forme sont de type `tile`. Dans ce cas, il existe un code ligne de `type=tile` lors de l’affichage des colonnes en mode Texte.
...
1. En mode texte pour une vue, ajoutez le texte suivant à la première colonne que vous souhaitez fusionner :
   ...
   **Exemple :** Le code suivant est le code du mode texte d’une colonne fusionnée qui contient trois colonnes distinctes, en commençant par la deuxième colonne de la liste. Les valeurs fusionnées sont Nom du projet, Date de début planifiée et Nom du propriétaire du projet et il n’y a pas de pause entre les trois valeurs :
...
1. Cliquez sur la première colonne et cliquez sur **Passer en mode Texte**, puis ajoutez le texte suivant à la colonne :

   `sharecol=true`

   Lorsque vous fusionnez les deux premières colonnes d’une liste ou d’un rapport, Workfront précède chaque ligne de texte qui contient des informations sur l’objet dans la première colonne avec `column.0.`, la colonne avec les informations de partage avec `column.1.` et les lignes de texte contenant des informations sur la deuxième colonne avec `column.2.`.
   ...
   **Exemple :** Le code suivant est le code du mode texte d’une colonne partagée qui contient le nom du projet, la date de début planifiée et le nom du propriétaire du projet avec un saut de ligne. La colonne partagée est la deuxième colonne d’une vue de projet.
