...
>[!IMPORTANT]
>
>Cet article nécessite une compréhension approfondie de l’API Adobe Workfront et de l’interface de création de rapports en mode texte. Pour plus d’informations sur l’API Workfront, reportez-vous à la section [Principes de base de l’API](../../../wf-api/general/api-basics.md).\
...
![Problème lié aux icônes du propriétaire de portefeuille](assets/issue-to-portfolio-owner-sraight-line-icons-350x83.png)

Pour obtenir la liste complète des objets dans Workfront, consultez l’ [API Explorer](../../../wf-api/general/api-explorer.md).
...
Pour plus d’informations sur la création de filtres complexes, consultez la section [Présentation des filtres de mode de texte complexe qui utilisent des instructions EXISTS](#overview-of-complex-text-mode-filters-that-use-exists-statements) de cet article.
...
* Vous devez utiliser le code d’objet de l’objet de liaison pour connecter vos instructions.\
  ...
  Pour plus d’informations sur l’Explorateur d’API, voir l’[ Explorateur d’API](../../../wf-api/general/api-explorer.md).
...
* Vous pouvez faire référence à plusieurs champs (champs cibles) sur le même objet (objet cible), auquel cas vous devez connecter les lignes faisant référence aux champs par AND.\
  Pour un exemple de filtrage pour plusieurs champs appartenant à l’objet cible, reportez-vous à la section [Exemple 4 : filtrer par plusieurs champs : tâches par nom de propriétaire du Portfolio et ID de Fiche d’évaluation d’alignement des Portfolios](#example-4-filter-by-multiple-fields-tasks-by-portfolio-owner-name-and-portfolio-alignment-scorecard-id) de cet article.
...
Pour des exemples de filtres, reportez-vous à la section [Exemple 1 : filtre pour les problèmes par nom de propriétaire de Portfolio](#example-1-filter-for-issues-by-portfolio-owner-name) de cet article.
...
1. À l’aide de l’explorateur d’API, identifiez le **code objet** de l’objet de liaison (projet).\
...
1. Collez l’exemple de formule suivant dans l’interface du mode texte du nouveau filtre et remplacez l’exemple de texte par les objets et les champs appropriés :
   ...
   Pour obtenir un exemple en utilisant les champs que nous avons identifiés ci-dessus, reportez-vous à la section [Exemple 1 : filtre des problèmes par nom de propriétaire de Portfolio](#example-1-filter-for-issues-by-portfolio-owner-name) de cet article.
...
1. (Conditionnel) Si vous filtrez les objets manquants, collez l’exemple de formule suivant dans l’interface du mode texte du nouveau filtre et remplacez le texte d’exemple par les objets et les champs appropriés :
   ...
   Pour un exemple de création de rapports sur les champs personnalisés qui ne sont pas associés à la Forms personnalisée, reportez-vous à la section [Exemple 2 : Filtre pour les objets manquants : champs personnalisés qui n&#39;apparaissent dans aucun formulaire personnalisé](#example-2-filter-for-missing-objects-custom-fields-that-do-not-appear-in-any-custom-forms) de cet article.
...
1. Collez le code suivant dans la zone **Définir les règles de filtrage pour votre rapport** pour remplacer le code générique ci-dessus :
...
1. Collez le code suivant dans la zone **Définir les règles de filtrage pour votre rapport** pour remplacer le code générique ci-dessus :
   ...
   >* L’instruction EXISTS:A:entryDate fait référence aux champs qui définissent l’objet cible (heure) et utilisent la même syntaxe que dans une instruction de filtre ordinaire. Vous affichez ainsi uniquement les utilisateurs qui n’ont pas consigné de temps pendant une période spécifique, dans ce cas la semaine précédente.
...
1. Cliquez sur **Passer en mode Texte**.
1. Collez le code suivant dans la zone **Définir les règles de filtrage pour votre rapport** :
